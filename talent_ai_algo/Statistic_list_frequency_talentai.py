...
def Statistic_list_frequency(u, v, type_values, parameters):
    ...
    for i in range(len(v)):
        ...
        if type_values[i] == "list":
            ...
            u_list = sorted(u[i]) #sorted(u_list, key=lambda x: parameters["list_freq_dict"][i][x], reverse=True)
            v_list = sorted(v[i]) #sorted(v_list, key=lambda x: parameters["list_freq_dict"][i][x], reverse=True)
            ...
            if len(v_list) < parameters["avg_list_len"][i]:
                v_list.extend([1] * (parameters["avg_list_len"][i] - len(v_list)))
            ...
            try:
                for j in range(len(u_list)):
                    f_v_ak = f_freq(specific_domain_size, theta1, betha, theta2, gamma)
                    fr_u = u_list[j] #parameters["list_freq_dict"][i][u_list[j]] if u_list[j] != "missing_val" else 1
                    fr_v = v_list[j]#parameters["list_freq_dict"][i][v_list[j]] if v_list[j] != "missing_val" else 1
